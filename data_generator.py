...
minimum_number_of_branches = 50
maximum_number_of_branches = 300

minimum_number_of_requests = 100
maximum_number_of_requests = 600

minimum_number_of_services = 2
maximum_number_of_services = 10

minimum_maximum_distance = 100
maximum_maximum_distance = 200

min_no_of_slots_for_one_service = 1
max_no_of_slots_for_one_service = 10

min_no_of_counters_at_branch = 3
max_no_of_counters_at_branch = 10

min_no_of_slots_per_day = 3
max_no_of_slots_per_day = 8
...
ratio_of_diagonal_of_whole_area_to_d = 50
...
def generate():
    ...
    for test_num in range(0, number_of_tests):
        ...
        path = "./general_dataset/test_" + str(test_num) + ".in"
        f = open(path, "w+")
        ...
        for i in range(m):
            ...
            for j in range(b):
                a = np.random.randint(
                    low=min_no_of_slots_for_one_service, high=max_no_of_slots_for_one_service)
                to_print += str(a) + " "
            f.write(to_print + "\n")
        ...
        for i in range(b):
            ...
            y = np.random.randint(low=0, high=max_y)
            s = np.random.randint(
                low=min_no_of_slots_per_day, high=max_no_of_slots_per_day)
            c = np.random.randint(
                low=min_no_of_counters_at_branch, high=max_no_of_counters_at_branch)
            f.write(str(x) + " " + str(y) + " " +
                    str(s) + " " + str(c) + " \n")
            for j in range(c):
